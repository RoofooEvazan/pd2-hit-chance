...
- Crit/CB deep dive (adv/re/crit_cb.md): crit sources SUM (344/347 weapon-matched + 337 + 258) then cap 75, one roll; DS only if crit fails. CB: one event node per (136, layer) (stat callback 0x6FCF9470); PD 0x102AF610 rolls once per hit (node logic VERIFIED 20k); efficiency = linear multiplier on the current-life share, applied before resist; no PD2 bug. Two-Hand Mastery (layer 2han via type2) counts only with the other hand empty (PD 0x102D30B0). PD2 Whirlwind: one hit per 5 frames max (PD 0x102BED80 replaces stock 0x6FC46E40), one target per hit (PD 0x102CB2C0).
