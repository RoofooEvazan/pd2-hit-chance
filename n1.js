...
/* PD2's own hit chance, PD 0x10271470 (VERIFIED natively, 8,180 cases, 0 mismatches). PD2 replaces the stock roll
   (D2Game 0x6FCFDE90, 5..95) for player and monster attacks (PD 0x10271740, roll = pdRand % 100, hit if < chance):
   ratio = 111·AR ÷ (AR + Def); chance = ratio·2·aLvl ÷ (aLvl + dLvl); below 5 → 5; above 95 the excess ratio counts
   at 90/111 (soft cap) and the result is capped at 100, so 100% is reachable. */
function hitChance(ar, alvl, def, dlvl) {
  if (def < 0) { ar -= def; def = 0; }
  if (ar < 0) { def -= ar; ar = 0; }
  const s = ar + def, L = alvl + dlvl;
  const ratio = s === 0 ? 100 : tdiv(111 * ar, s);
  if (L === 0) return 5;
  const c = tdiv(ratio * alvl * 2, L);
  if (c < 5) return 5;
  if (c <= 95) return c;
  let ex = ratio - tdiv(95 * L, 2 * alvl);
  if (s) ex = tdiv(tdiv(s * ex, 111) * 90, s);
  return Math.min(tdiv(ex * alvl * 2, L) + 95, 100);
}
